# arci development discipline

This file encodes arci's development approach as instructions for Claude Code. It's the Stage 0 substitute for hooks, skills, and enforcement policies that don't exist yet. Follow these instructions when working on arci.

## Before starting work

Check what's ready. The knowledge graph in `.arci/graph.jsonlt` tracks all work as TASK-* nodes in a dependency DAG. Before picking up work, find tasks whose dependencies are all complete:
...
| Rule | Classification | Stage | Replacement |
|------|---------------|-------|-------------|
| Run `just lint` before committing | Temporary | 2 | Pre-commit hook via arci hooks |
| Run `just format` to auto-fix | Temporary | 2 | Pre-commit hook via arci hooks |
...
When a task is complete, update its status and record its deliverables. Edit the task's entry in `.arci/graph.jsonlt`:
...
| Rule | Classification | Stage | Replacement |
|------|---------------|-------|-------------|
| Set status to complete and add timestamp | Temporary | 1 | `arci task complete` CLI command |
| Record deliverables with kind discriminator | Temporary | 1 | `arci task complete --deliverable` CLI command |
| Set in-progress status with summary | Temporary | 1 | `arci task start` CLI command |
...
| Manual graph editing to create defect nodes | Temporary | 1 | `arci defect create` CLI command |
...
| Rule | Classification | Stage | Replacement |
|------|---------------|-------|-------------|
| Reference task ID in commit messages | Temporary | 2 | Git commit hook via arci hooks |
| Use conventional commits format | Temporary | 2 | Commitlint via arci hooks |
| One coherent unit of work per commit | Permanent | n/a | n/a |

## Graph file format

`.arci/graph.jsonlt` stores the knowledge graph as a JSON Lines file where each line is a JSON-LD compact form document representing one node. The file is the single source of truth for all structured data. Prose files have no frontmatter.
...
| 8-character Crockford Base32 nanoids | Temporary | 1 | `arci` CLI nanoid generation |
...
Extended prose for nodes lives in flat directories under `.arci/`:
...
| Rule | Classification | Stage | Replacement |
|------|---------------|-------|-------------|
| Prose in flat directories under `.arci/` | Permanent | n/a | n/a |
| Filename convention with timestamp and nanoid | Temporary | 1 | `arci` CLI file generation |
| Nanoid in filename matches @id | Temporary | 1 | `arci` CLI file generation |
...
| Don't hand-edit graph without schema knowledge | Temporary | 1 | `arci` CLI with schema-aware validation |
...
`docs/plans/build-arci-with-arci.md` is the bootstrapping plan.
