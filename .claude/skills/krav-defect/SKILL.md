...
replacement-stage: 1
replacement: "`krav defect` CLI subcommands (create, confirm, defer, reject)"
---
...
2. If the defect lacks a prose file at `.krav/defects/{timestamp}-{NANOID}-{slug}.md`, create one during triage. Include the full problem description, evidence, reproduction context, and the reasoning behind the disposition decision.
...
4. For confirmed defects, create a remediation TASK-* node and link it with `generates`. Create a prose file for the task at `.krav/tasks/{timestamp}-{NANOID}-{slug}.md` with the remediation approach and constraints.
...
|---------|---------------|-------|-------------|
| Open defects by module context query | Temporary | 1 | `krav defect list` CLI command |
