...
Review configuration with `krav config validate`. The server does not start with invalid configuration.

Run with verbose logging: `KRAV_LOG_LEVEL=debug krav server`
...
In direct execution mode (no server), configuration loads fresh on every invocation. If changes are not reflected, check that you are editing the correct file. Use `krav config list` to see which files the system loads.

### Stale lockfile

If `.krav/server.json` exists but the server isn't running, any `krav` command detects the stale PID and cleans up the file automatically. No manual intervention is necessary. See [discovery](discovery.md) for details.

## Error recovery

Krav provides automatic recovery for transient failures and tools for manual recovery of persistent issues.
...
Corrupted state store: `krav state clear --all` wipes the state database. For complete reset, delete the DuckDB file at `.krav/state.duckdb`.

Unresponsive server: press Ctrl+C in the terminal where the server runs for a graceful shutdown. If the server is truly hung, `kill <pid>` (reading the PID from `.krav/server.json`) forces termination. The next `krav` command cleans up the stale lockfile.

Extension conflicts: `krav extension sync` reinstalls extensions from the lockfile. For persistent issues, `krav extension remove <name>` and re-add.
...
# HELP krav_evaluations_total Total number of hook evaluations
# TYPE krav_evaluations_total counter
krav_evaluations_total 1523

# HELP krav_evaluation_errors_total Total evaluation errors by type
# TYPE krav_evaluation_errors_total counter
krav_evaluation_errors_total{error_type="expression_error"} 3
krav_evaluation_errors_total{error_type="state_error"} 1
```
