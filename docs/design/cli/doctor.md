...
- **logs**: the system can write to the project-level log location

Each check reports pass, warning, or fail status with details about any issues found.

## Options

- `--fix`: attempt to automatically repair common issues where possible (such as recreating a corrupted state store, re-running `arci install` for misconfigured Claude Code integration). Prompts for confirmation before making changes.
- `--yes`: skip confirmation prompts when used with `--fix`.
- `--check <name>`: run only specific checks. The user can repeat this flag to run multiple checks. Valid names: `installation`, `claude-code`, `config`, `policies`, `state`, `extensions`, `logs`.
