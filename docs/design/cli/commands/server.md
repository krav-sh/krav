...
Starts the Krav server for the current project.
...
The server binds to localhost on the configured base port (default 7680). If the port is in use, it tries incrementing ports up to 20 times. The server writes the actual port to `.krav/server.json` for discovery by other `krav` commands.
...
`--log-level <level>`: logging verbosity. Values: `error`, `warn`, `info`, `debug`. Default: `info`. Also settable via `KRAV_LOG_LEVEL`.
