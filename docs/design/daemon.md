...
In direct execution mode, `arci hook apply` loads configuration, compiles policy expressions, and evaluates policies on every invocation. This is the default and requires no setup beyond installing arci and writing policies. The overhead is typically 50-200ms depending on configuration complexity.
...
Direct execution suits users who are getting started with arci, have a small number of policies (under ~50), don't need the diagnostics dashboard, or want the simplest possible setup. The daemon suits users with many policies or complex expressions who want sub-10ms evaluation latency, need the diagnostics dashboard for debugging, or want hot-reload without restarting anything.
...
The daemon owns the core evaluation engine. Policy expressions are compiled once and reused for each evaluation. This avoids the overhead of re-parsing expressions on every hook invocation.
...
Metrics accumulation happens in memory. The daemon tracks policy match counts, action executions, errors, and timing information. These metrics are exposed via the API and displayed in the dashboard.
...
The daemon is built on Go's `net/http` with the `chi` router and `nhooyr.io/websocket`. Chi provides composable, lightweight HTTP routing. Go's goroutines and channels handle concurrent connections efficiently without a separate async runtime.
...
The primary endpoint for hook evaluation. The CLI calls this for every hook invocation when daemon mode is enabled.
...
Serves the dashboard web interface. Returns HTML pages rendered with Go templates. Uses htmx for dynamic updates.
...
Configuration loading involves discovering sources, reading and parsing YAML files, merging policies by precedence, compiling policy expressions, and caching the result. Configuration for specific projects is loaded lazily on first request, keeping startup fast even with many potential projects.

The file watcher monitors all configuration source directories using the `fsnotify` package. When a change is detected, the affected project's configuration is marked dirty. On the next request for that project (or immediately if configured), the configuration is reloaded.
...
The daemon manages SQLite connections for the state store. Each project has its own database file. Connections are pooled for reuse. Session-scoped state is stored in the project's database with the session ID. Project-scoped state uses an empty session ID.
...
Metrics are reset on daemon restart. Persistent metrics could be added later, but in-memory is sufficient for diagnostics. The metrics endpoint returns a snapshot. The dashboard polls this periodically or uses the WebSocket for live updates.

## Lifecycle

The daemon runs in the foreground via `arci daemon start`. It logs to stderr and can be stopped with Ctrl+C. The daemon handles SIGTERM gracefully, completing in-flight requests before shutting down.
...
When daemon mode is enabled in configuration, `arci hook apply` first attempts to connect to the daemon. If the daemon is reachable, evaluation is delegated. If not, behavior depends on the `auto_start` setting: either start the daemon automatically, fall back to direct execution, or fail with an error.
...
On modern Linux distributions, systemd is ubiquitous. User services (via `systemctl --user`) don't require root and integrate well with desktop sessions. Socket activation is a compelling feature for on-demand startup.

On macOS, launchd is the native service manager. Launch agents run at user login and are managed through `launchctl`. Launchd supports on-demand socket activation through its own mechanism.
...
The MCP server exposes diagnostic tools for querying status, listing active policies for the current project, and retrieving evaluation statistics. These tools provide introspection capabilities without requiring the daemon to be running—if the daemon isn't running when the MCP server starts, it reports an error through MCP rather than attempting to spawn the daemon itself. This keeps lifecycle management cleanly separated.

## Error isolation

The daemon isolates errors to prevent cascading failures. Evaluation errors for one request don't affect other requests. Configuration errors for one project don't affect other projects. Individual rule or action failures are logged but don't fail the evaluation.
...
Log levels are configurable via the `ARCI_LOG_LEVEL` environment variable. Debug level logs detailed information about every evaluation. Info level logs significant events like configuration reloads. Warning level logs recoverable errors. Error level logs failures that require attention.
