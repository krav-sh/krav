...
Tool access allows only reads: Read, Grep, Glob, and Bash (for running `arci` CLI commands to query the graph and create defects). No Write or Edit access. The reviewer can read any file in the project but can't modify anything except the graph (via CLI commands that create defect nodes).

Preloaded skills: `arci:review`. The review skill's !`command` preprocessing injects the module's requirements and deliverable list before the reviewer sees its instructions. The skill's instructed commands tell the reviewer to run `arci defect create` for each problem found.
...
Tool access: Read, Grep, Glob, and Bash. Bash access is broader than the reviewer's because the verifier needs to execute test suites, but Write and Edit are still excluded. The verifier records results through `arci` CLI commands, not by editing test files.

Preloaded skills: `arci:verify`. Preprocessing injects the module's test cases, their current results, and the verification method for each. The skill instructions guide the verifier through executing each test case and recording pass/fail/skip results with evidence.
