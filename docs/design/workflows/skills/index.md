...
This directory contains design specifications for the skills that ARCI ships as part of its Claude Code plugin. Each skill encodes a workflow from the [workflow index](../index.md) as structured instructions that Claude Code follows during execution.

Skills live in the plugin's `claude/skills/` directory and are namespaced as `arci:skill-name` to avoid conflicts with project or personal skills (so the `init` skill gets invoked as `arci:init`). Each skill has YAML frontmatter (tool restrictions, invocation control, hooks, model overrides) and a markdown body with workflow instructions. Skills use `!`command`` preprocessing to inject graph context at load time and instructed commands for interactive graph queries during execution.
...
| review           | [Reviewing work](../reviewing-work.md)                               | Evaluates deliverables against requirements, produces defects for problems found. Runs inside the `arci-reviewer` subagent. Preprocessing loads the module's requirements and deliverable file list. Instructs the agent to create defects via CLI for each problem. |
| verify           | [Running verification](../running-verification.md)                   | Executes test cases and records results on TC-\* nodes. Runs inside the `arci-verifier` subagent. Preprocessing loads the module's test cases and their current results.                                                                                             |
...
| diff | [Comparing baselines](../comparing-baselines.md) | Produces a semantic diff between two baselines: nodes added, removed, changed; edges created, broken; status transitions. Runs inside the `arci-analyst` subagent for larger comparisons. |
